...
| `ActionTagParser.swift` | ~168 | Unified parser for all action tags Claude can embed in responses: `[LOG:app:topic]`, `[OPEN:url-or-app]`, `[SHORTCUT:name]`, `[REMIND:text:date-hint]`. Returns `ParsedActionTags` with cleaned text and extracted actions. NOTE tags are stripped upstream by `CompanionManager.parseAndStripNoteTags` before this runs. |
