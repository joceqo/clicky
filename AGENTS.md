...
**`clicky://` URL scheme**: Registered via `CFBundleURLTypes` in Info.plist. Handled in `CompanionAppDelegate.handleClickyURL`. Currently implements `clicky://speak?file=<path>` and `clicky://speak?text=<urlencoded>`, which route through `CompanionManager.speakExternalText` and the markdown-stripping TTS pipeline. Used by the Claude Code Stop hook (`~/.claude/hooks/clicky-speak.sh`) so Clicky reads Claude Code assistant replies aloud.
...
| `AppExtractionHeuristicsStore.swift` | ~155 | Per-app outcome cache for read-aloud text extraction. Records AX success/failure and word counts per bundle ID, persists to UserDefaults, and lets the read-aloud pipeline skip AX entirely for apps we've observed return nothing useful (Chrome, Electron, VS Code). Entries age out after 7 days; AX-failure cache resets after 5 min so transient breakage self-heals. |
| `ReadAloudSessionLog.swift` | ~215 | Persistent JSONL log of every ⌃⇧L read-aloud session. Writes one record per session to `~/Library/Application Support/Clicky/logs/read-aloud.jsonl` on a background queue: app context, extraction strategy, word counts, phase latencies (extraction / synthesis / end-to-end), outcome (played / stoppedByUser / error), and error details. Rotates files older than 7 days; keeps the last 4 archives. Built so a human (or another agent) can come back days later and spot patterns across apps/strategies/errors. |
...
| `TTSTextCleaner.swift` | ~200 | Strips markdown, code fences, HTML, and emphasis markers from text before it hits ElevenLabs/Supertonic so responses sound like prose instead of "asterisk asterisk bold". Applied at `CompanionManager.speakTextWithActiveTTSProvider`. |
